...
if st.button("Get Forecast"):
    ...
    response = requests.post("http://fastapi:8000/forecast", json=
    {"ticker": ticker, "periods" : forecast_days})
    if response.status_code == 200:
        ...
        combined_data = pd.concat([historical[['ds','y','type']],
        ...
        line_chart = alt.Chart(combined_data).mark_line().encode(
            x=alt.X('ds:T', title='Date', axis=alt.Axis(format='%Y-%m-%d',
            labelFontSize=18, titleFontSize=20)),
            y=alt.Y('y:Q', title='Stock Price (USD)',
